...
"use strict";function GetElements(){cboCategory=$("#cboCategory"),cboDifficulty=$("#cboDifficulty"),cboType=$("#cboType"),btnShoot=$("#btnShoot"),btnPrefs=$("#btnPrefs"),btnClose=$("#btnClose"),main__content=$(".main__content"),header__navigation=$(".header__navigation"),header__slogan=$(".header__slogan"),main__content__container=$(".main__content__container"),main__content__buttons=$(".main__content__buttons"),main__content__preferences=$(".main__content__preferences"),main__content__question=$(".main__content__question"),main__content__answers=$(".main__content__answers"),main__content__response=$(".main__content__response")}function SetupCategories(n){$.getJSON(n,function(n){ShowCategories(n)})}function ShowCategories(n){var e=n.trivia_categories;FillCategoriesComboBox(e=SortCategories(e))}function SortCategories(n){return n.sort(function(n,e){return n.name<e.name?1:n.name>e.name?-1:0}),n}function FillCategoriesComboBox(n){for(var e,t=n.length-1;t>=0;t--)void 0!==t&&(e+='<option value="'+n[t].id+'">'+n[t].name+"</option>");cboCategory.append(e)}function ShowPreferences(){main__content__preferences.css("display","flex"),main__content__buttons.css("display","none"),$(document).width()<560&&header__slogan.css("display","none")}function ClosePreferences(){main__content__preferences.css("display","none"),main__content__buttons.css("display","flex"),$(document).width()<560&&header__slogan.css("display","block")}function Shoot(){var n={category:cboCategory.val(),difficulty:cboDifficulty.val(),type:cboType.val()};firstTime?(GetQuestionsAndShow(GetUrlQuestion(n)),firstTime=!1):(showQuestion(),arrQuestions.length<2&&GetQuestions(GetUrlQuestion(n)))}function GetUrlQuestion(n){var e=_GETQUESTION_LINK;for(var t in n)""!=n[t]&&(e+="&"+t+"="+n[t]);return e}function GetQuestionsAndShow(n){$.getJSON(n,function(n){if(0!=n.response_code)return console.log("Error, try again"),null;arrQuestions=arrQuestions.concat(n.results),showQuestion()})}function GetQuestions(n){$.getJSON(n,function(n){if(0!=n.response_code)return console.log("Error, try again"),null;arrQuestions=arrQuestions.concat(n.results)})}function hideOtherContainersButQuestion(){changeDisplayProperty(main__content__container,"none"),changeDisplayProperty(header__slogan,"none"),changeDisplayProperty($(".main__content__answers"),"grid"),changeDisplayProperty($(".main__content__question"),"block"),$(document).width()<580&&header__navigation.css("display","none")}function showQuestion(){hideOtherContainersButQuestion();var n=arrQuestions[0];if(console.log(n),main__content__question.empty(),main__content__question.append('<div class="question__card"><p class="question__card__category">'+n.category+'</p><p class="question__card__question">'+n.question+"</p></div>"),main__content__answers.empty(),"multiple"==n.type){var e=n.incorrect_answers;e.push(n.correct_answer),e.sort();for(var t=0;t<e.length;t++)createButtonAnswer(e[t],main__content__answers)}else createButtonAnswer("True",main__content__answers),createButtonAnswer("False",main__content__answers);$(".btnAnswer").on("click",function(e){checkQuestion(e.target.innerHTML,n.correct_answer)})}function createButtonAnswer(n,e){e.append('<button class="btnAnswer">'+n+"</button>")}function checkQuestion(n,e){hideQuestion(),header__navigation.css("display","block"),main__content__response.empty(),main__content__response.css("display","grid"),n==e?showCorrect():showIncorrect(e),$(".btnResponse").click(newQuestion)}function hideQuestion(){main__content__answers.css("display","none"),main__content__question.css("display","none")}function newQuestion(){arrQuestions.shift(),main__content.removeClass("correct incorrect"),changeDisplayProperty(main__content__response,"none"),Shoot()}function showCorrect(){main__content__response.append($('<?xml version="1.0" encoding="UTF-8"?><svg viewBox="0 0 148.11 119.31" xmlns="http://www.w3.org/2000/svg"><defs><style>.svgcolor{fill:#cecece;}</style></defs><g data-name="Layer 2"><g data-name="Eindwerk"><g data-name="QuestionSucces"><polygon class="svgcolor" points="125.98 0 50.94 75.05 22.12 46.24 0 68.36 28.81 97.17 28.8 97.18 50.92 119.31 148.11 22.12"/></g></g></g></svg>'),$("<h1/>",{text:getRandomResponse(responseCorr)}),$("<button/>",{class:"btnResponse",text:"New question!"})),main__content.addClass("correct")}function showIncorrect(n){main__content__response.append($('<?xml version="1.0" encoding="UTF-8"?><svg viewBox="0 0 119.7 120.7" xmlns="http://www.w3.org/2000/svg"><defs><style>.svgcolor{fill:#aa1818;}</style></defs><title>Asset 10</title><g data-name="Layer 2"><g data-name="Eindwerk"><g data-name="QuestionSucces2"><polygon class="svgcolor" points="119.7 23.13 97.57 1 60.35 38.22 22.13 0 0 22.13 38.22 60.35 0 98.57 22.13 120.7 60.35 82.48 97.57 119.7 119.7 97.57 82.48 60.35"/></g></g></g></svg>'),$("<canvas/>",{height:160}),$("<div/>").append($("<h1/>",{text:getRandomResponse(responseInCorr)}),$("<p/>",{text:"The correct answer was: "+n})),$("<button/>",{class:"btnResponse",text:"New question!"})),main__content.addClass("incorrect")}function changeDisplayProperty(n,e){n.css("display",e)}function getRandomResponse(n){return n[Math.floor(Math.random()*n.length)]}var cboCategory,cboDifficulty,cboType,btnShoot,btnPrefs,btnClose,main__content,header__navigation,header__slogan,main__content__buttons,main__content__preferences,main__content__question,main__content__container,main__content__answers,main__content__response,_AMOUNT_QUESTIONS=5,_GETALLCATEGORIES_LINK="https://opentdb.com/api_category.php",_GETQUESTION_LINK="https://opentdb.com/api.php?amount="+_AMOUNT_QUESTIONS,arrQuestions=[],firstTime=!0;responseCorr=["Great, succes!","You‘re a real smart ass!","That‘s correct!","Hell yeah!","You are from another planet!"],responseInCorr=["Let‘s pretend that never happened!","Oops, you pressed the wrong button!","Have you read the question properly?","Try again, dumbass.","Wrong answer!"],document.addEventListener("DOMContentLoaded",function(){console.log("Let's start, press the shoot button!"),GetElements(),SetupCategories(_GETALLCATEGORIES_LINK),btnShoot.click(Shoot),btnPrefs.click(ShowPreferences),btnClose.click(ClosePreferences)});